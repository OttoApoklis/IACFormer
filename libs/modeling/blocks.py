...
class LocalMaskedMHCA(nn.Module):
    ...
    def __init__(
        ...
        # 1
        print("n_qx_stride:", n_qx_stride)
        # 1
        print("n_kv_stride:", n_kv_stride)
        super().__init__()
        assert n_embd % n_head == 0
        self.n_embd = n_embd
        self.n_head = n_head
...
class TransformerBlock(nn.Module):
    ...
    def forward(self, x, mask, pos_embd=None):
        ...
        out1, out_mask1 = self.attn1(self.ln1(x), mask)
        out2, out_mask2 = self.attn2(self.ln1(x), mask)
        out = out + torch.relu(out1 * torch.relu(out2.mean(dim=-1, keepdim=True)))
        out_mask_type = out_mask.dtype
        out_mask = out_mask + torch.relu(out_mask1.to(out.dtype) * torch.relu(out_mask2.to(out.dtype).mean(dim=-1, keepdim=True)))
        out_mask = out_mask.to(out_mask_type)
...
class SGPBlock(nn.Module):
    ...
    def __init__(
        ...
        # assert (kernel_size % 2 == 1) and (kernel_size > 1)
        # padding = kernel_size // 2
        self.add_new_direct = add_new_direct
